...
import Markdown from 'react-markdown'//for structured details
  ...
  return (
    <div onClick={()=>setExpanded(!expanded)} className='p-4 max-w-5xl text-sm bg-white border border-gray-200 rounded-lg cursor-pointer'>
        <div className='flex justify-between items-center gap-4'>
            ...
            <button className='bg-[#EFF6FF] border border-[#A0522D] text-[#A52A2A] px-4 py-1 rounded-full'>{item.type}</button>
        </div>
        {
            expanded && (
                <div>
